...
{ 
	...
	{
		if(len==1)
			*buf=IIC3_Read_Byte(0);//读数据,发送nACK 
		else 
			*buf=IIC3_Read_Byte(1);		//读数据,发送ACK 
		len--;
		buf++; 
	}    
    IIC3_Stop();	//产生一个停止条件 
...
{ 
  IIC3_Start(); 
	IIC3_Send_Byte((VL53L0X_Add<<1)|0);//发送器件地址+写命令	
	if(IIC3_Wait_Ack())	//等待应答
	{
		IIC3_Stop();		 
		return 1;		
...
u8 VL53L03X_Read_Byte(u8 reg)
{
  ...
  IIC3_Start(); 
	...
	IIC3_Send_Byte(0x52);//发送器件地址+写命令	
	IIC3_Wait_Ack();		//等待应答 
  IIC3_Send_Byte(reg);	//写寄存器地址
  IIC3_Wait_Ack();		//等待应答
  IIC3_Start();
	IIC3_Send_Byte(0x53 );//发送器件地址+读命令	
  IIC3_Wait_Ack();		//等待应答 
